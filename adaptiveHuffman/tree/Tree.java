...
public class Tree {
	 ...
	 * @param node 
	 */
	private void printTreeDepth(Node node, int level) {
		if (node.isNYT()) {
			printNode("  NYT   ", level);
		} else if (node.isLeaf()) {
			char character = (char) node.getValue();
			printNode("Leaf: " + character + " weight "+ node.getWeight(), level);
		} else { // Node is an internal node.
			printNode("Internal Node: " + node.getWeight(), level);
			printTreeDepth(node.left, level + 1);
			printTreeDepth(node.right, level + 1);
		}
	}

	private void printNode(String label, int level) {
